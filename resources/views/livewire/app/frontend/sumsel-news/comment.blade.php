<div>
    <section class="mb-3" id="comment-box">
        <h2>Komentar ({{ $sumselNews->totalCommentedByUsers() }})</h2>
                                        ...
                                        @auth
                                            <a wire:key="'like-comment-'{{ $comment->id }}" wire:click='toggleLike({{ $comment->id }})'
                                                ...
                                                <svg wire:loading.delay.remove wire:target='toggleLike({{ $comment->id }})'
                                                    ...
                                                    class="icon icon-tabler icon-tabler-heart {{ in_array(auth()->id(), $comment->likeCommentSumselNews->pluck('user_id')->toArray()) ? 'text-danger' : 'text-muted' }}"
                                                        ...
                                                        @auth
                                                            <a wire:key="'like-reply-'{{ $firstLvlReply->id }}"
                                                                ...
                                                                <svg wire:loading.delay.remove wire:target='toggleLike({{ $firstLvlReply->id }})'
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                                    class="icon icon-tabler icon-tabler-heart {{ in_array(auth()->id(), $firstLvlReply->likeCommentSumselNews->pluck('user_id')->toArray()) ? 'text-danger' : 'text-muted' }}"
