        ...
        <table class="table table-vcenter card-table table-striped table-hover">
            <thead class="text-nowrap">
                <tr>
                    <th class="w-7">No.</th>
                    <th>
                        <span class="d-inline-block py-1">Nama Permission</span>
                    </th>
                    <th>
            ...
            <tbody>
                @forelse ($this->permissions as $permission)
                    <tr wire:key='{{ $permission->id }}'>
                        ...
                        <td class="text-muted">
                            {{ $permission->created_at->format('d M Y') . ', ' . $permission->created_at->format('H:i') }}
                        </td>
                        <td>
                            <div class="btn-list flex-nowrap">
                                <button type="button" x-on:click="$dispatch('set-data', { slug: '{{ $permission->slug }}' })"
                                ...
                                <a x-on:click="$dispatch('delete-confirm', { id: {{ $permission->id }}, name: '{{ $permission->name }}' })"
