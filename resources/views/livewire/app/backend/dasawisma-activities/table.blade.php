<div>
    ...
    <div wire:loading.class='invisible' class="table-responsive">
        <table class="table table-vcenter card-table table-striped table-hover">
            ...
            <tbody>
                @forelse ($this->dasawismaActivities as $dasawismaActivity)
                    <tr wire:key='{{ $dasawismaActivity->id }}'>
                        <th class="text-muted">
                            {{ ($this->dasawismaActivities->currentPage() - 1) * $this->dasawismaActivities->perPage() + $loop->iteration }}
                        </th>
                        <td>
                            <div class="avatar avatar-md" style="background-image: url({{ $dasawismaActivity->image }}); width: 6.5rem"></div>
                        </td>
                        <td>{{ $dasawismaActivity->title }}</td>
                        <td class="text-muted">{{ $dasawismaActivity->getReadingTime() }}</td>
                        <td class="text-muted">{{ $dasawismaActivity->author }}</td>
                        <td class="text-muted">
                            {{ $dasawismaActivity->created_at->format('d M Y') . ', ' . $dasawismaActivity->created_at->format('H:i') }}
                        </td>
                        @if (auth()->user()->role_id != 3)
                            <td class="text-muted">
                                <livewire:app.bootstraps.toggle-button :model="$dasawismaActivity" field="is_published" :key="$dasawismaActivity->id" />
                            </td>
                        @endif
                        @if (auth()->user()->role_id != 3)
                            <td>
                                <div class="btn-list flex-nowrap">
                                    <a wire:navigate href="{{ route('area.dasawisma_activity.edit', $dasawismaActivity->slug) }}"
                                    ...
                                    <a x-on:click="$dispatch('delete-confirm', { id: {{ $dasawismaActivity->id }}, title: '{{ $dasawismaActivity->title }}' })"
