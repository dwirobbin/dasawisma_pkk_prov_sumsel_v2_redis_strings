        ...
        switch (true) {
            case $param == 'index': // regencies
                ...
                if ($this->search != '') {
                    ...
                    if (Redis::exists($keyCacheFNRegencies)) {
                        $cachedData = Redis::get($keyCacheFNRegencies);
                        $familySizeMembers = LengthPager::paginate(json_decode($cachedData, true));
                    } else {
                        ...
                        if ($familySizeMembers->isNotEmpty()) {
                            Redis::set($keyCacheFNRegencies, json_encode($familySizeMembers), 'EX', config('database.redis.options.ttl'));
                        }
                    }
                }
                break;
            case strlen($param) == 4: // districts
                ...
                if ($this->search != '') {
                    ...
                    if (Redis::exists($keyCacheFNDistricts)) {
                        $cachedData = Redis::get($keyCacheFNDistricts);
                        $familySizeMembers = LengthPager::paginate(json_decode($cachedData, true));
                    } else {
                        $familySizeMembers = $this->getData()
                            ->addSelect('districts.id', 'districts.name')
                        ...
                        if ($familySizeMembers->isNotEmpty()) {
                            Redis::set($keyCacheFNDistricts, json_encode($familySizeMembers), 'EX', config('database.redis.options.ttl'));
                        }
                    }
                }
                break;
            case strlen($param) == 7: // villages
                ...
                if ($this->search != '') {
                    ...
                    if (Redis::exists($keyCacheFNVillages)) {
                        $cachedData = Redis::get($keyCacheFNVillages);
                        $familySizeMembers = LengthPager::paginate(json_decode($cachedData, true));
                    } else {
                        $familySizeMembers = $this->getData()
                            ->addSelect('villages.id', 'villages.name')
                        ...
                        if ($familySizeMembers->isNotEmpty()) {
                            Redis::set($keyCacheFNVillages, json_encode($familySizeMembers), 'EX', config('database.redis.options.ttl'));
                        }
                    }
                }
                break;
            case strlen($param) == 10: // dasawisma's
                ...
                if ($this->search != '') {
                    ...
                    if (Redis::exists($keyCacheFNDasawismas)) {
                        $cachedData = Redis::get($keyCacheFNDasawismas);
                        $familySizeMembers = LengthPager::paginate(json_decode($cachedData, true));
                    } else {
                        $familySizeMembers = $this->getData()
                            ->addSelect('dasawismas.id', 'dasawismas.name', 'dasawismas.slug')
                        ...
                        if ($familySizeMembers->isNotEmpty()) {
                            Redis::set($keyCacheFNDasawismas, json_encode($familySizeMembers), 'EX', config('database.redis.options.ttl'));
                        }
                    }
                }
                break;
            case $param == 'dasawisma': // family_heads
                ...
                if ($this->search != '') {
                    ...
                    if (Redis::exists($keyCacheFNFamilies)) {
                        $cachedData = Redis::get($keyCacheFNFamilies);
                        $familySizeMembers = LengthPager::paginate(json_decode($cachedData, true));
                    } else {
                        $familySizeMembers = $this->getData()
                            ->addSelect('family_heads.id', 'family_heads.family_head AS name')
                        ...
                        if ($familySizeMembers->isNotEmpty()) {
                            Redis::set($keyCacheFNFamilies, json_encode($familySizeMembers), 'EX', config('database.redis.options.ttl'));
                        }
                    }
                }
                break;
